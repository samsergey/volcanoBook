...
function PSD(data)
{
    var N = Math.pow(2,Math.ceil(Math.log(data.length)/Math.log(2)))
    var d = data.concat(data.slice(0,N-data.length))
    return cfft(d).slice(1,N/2+1).map(z => z.module()/d.length)
}
//test code
